...
var PMO = {
	...
	listOrgs: function() {
		Fluid.releaseFrom(1);
		var c = Fluid.addColumn({width:250, title:"Organisations"});
	...
	viewHome: function() {
		Fluid.releaseFrom(2);
		Fluid.addColumn({width:500, title:"Home"});
	},

	viewPersonal: function() {
		Fluid.releaseFrom(2);
		//Fluid.addColumn({width:500, title:"Personal"});
		PMO.viewProject(1);
	},
	...
	viewOrg: function(org) {
		...
		var request = new AjaxRequest2({
			...
			onSuccess: function(data) {
				var org = data[0];
				var c = Fluid.addColumn({width:280, title: org.name});
	...
	listPrograms: function(org) {
		...
		c = Fluid.addListColumn({width:430, title:"Programs", data:data, render:function(node, item) {
	...
	listPeople: function(org) {
		Fluid.releaseFrom(3);
		Fluid.addColumn({width:280, title:"People"});
	},
	...
	viewProgram: function(id) {
		...
		var request = new AjaxRequest2({
			...
			onSuccess: function(data) {
				var p = data[0];
				var c = Fluid.addColumn({width:280, title:p.name});
				var buttons = new Array();
	...
	listProjects: function(id) {
		...
		c = Fluid.addListColumn({width:430, title:"Projects", data:data, render:function(node, item) {
			node.onclick = function() {
	...
	viewStatus: function(id) {
		Fluid.releaseFrom(7);
		Fluid.addColumn({width:500, title:"Status", isSubtitle:true});
	}
};
